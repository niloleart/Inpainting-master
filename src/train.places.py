...
if not os.path.exists( trainset_path ) or not os.path.exists( testset_path ):
    ...
    for dir, _, _, in os.walk(dataset_path):
        imagenet_images.extend( glob( os.path.join(dir, '*.png')))

    for dir, _, _, in os.walk(mask_path):
        mask_images.extend( glob( os.path.join(dir, '*.png')))

    if int(len(mask_images))!=int(len(imagenet_images)):
        print("             MASK DATASET SIZE IS DIFFERENT FROM THE ORIGINAL ONE")

    imagenet_images = np.hstack(imagenet_images)
    mask_images = np.hstack(mask_images)
    trainset = pd.DataFrame({'image_path':imagenet_images[:int(len(imagenet_images)*0.9)],
                             'mask_path':mask_images[:int(len(mask_images)*0.9)]})
...
#changed reuse=True to =tf.AUTO_REUSE
adversarial_neg = model.build_adversarial(reconstruction, is_train, reuse=tf.AUTO_REUSE)
...
for epoch in range(n_epochs):
    ...
    for start,end in zip(
        ...
        # instead of direct load_image for images_ori, replace appropriate stuff in image_paths, so that paths point to masks instead of orig images
        images_ori = map(lambda x: load_image_2(x), image_paths)
        #load masks
        mask_paths = trainset[start:end]['mask_path'].values

        masks = map(lambda y: load_image_2(y), mask_paths)

        is_none = np.sum(map(lambda x: x is None, images_ori))
        if is_none > 0: continue

        #images_crops = map(lambda x: crop_random(x), images_ori)
        images_crops = map(lambda x,y: merge_mask(x, y), images_ori, masks)
        ...
        if iters % 100 == 0:
            ...
            test_images_ori = map(lambda x: load_image_2(x), test_image_paths)

            test_mask_paths = testset[:batch_size]['mask_path'].values
            test_masks_ori = map(lambda y: load_image_2(y), test_mask_paths)
