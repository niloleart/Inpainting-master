...
dataset_path = '/media/storage3/Study/data/lsun/'
model_path = '../models/lsun/'
result_path= '../results/lsun/'
pretrained_model_path = '../models/lsun/model-1' #None
...
if not os.path.exists( trainset_path ) or not os.path.exists( testset_path ):
    ...
    for dir, _, _, in os.walk(dataset_path):
        lsun_images.extend( glob( os.path.join(dir, '*.jpg')))

    lsun_images = np.hstack(lsun_images)

    trainset = pd.DataFrame({'image_path':lsun_images[:int(len(lsun_images)*0.9)]})
    testset = pd.DataFrame({'image_path':lsun_images[int(len(lsun_images)*0.9):]})
...
loss_recon_center = tf.reduce_mean(tf.sqrt( tf.reduce_sum(loss_recon_ori * mask_recon, [1,2,3])))  # Loss for non-overlapping region
loss_recon_overlap = tf.reduce_mean(tf.sqrt( tf.reduce_sum(loss_recon_ori * mask_overlap, [1,2,3]))) * 10.# Loss for overlapping region
        ...
        if iters % 100 == 0:
            ...
            if iters % 1000 == 0:
                ...
                for rec_val, img,x,y in zip(reconstruction_vals, test_images, xs, ys):
                    ...
                    cv2.imwrite( os.path.join(result_path, 'img_'+str(ii)+'.'+str(int(iters/1000))+'.jpg'), rec_con)
                    ii += 1
                    if ii > 30: break
